...
DRESULT disk_read (BYTE pdrv, BYTE* buff, DWORD sector, UINT count)
{
    ...
    /* TODO */
	if(ide_read_sectors(DISK_ID, count, sector, (unsigned int)buff) == 0){
		return RES_OK;
	}
	else{
		return -RES_ERROR;
...
DRESULT disk_write (BYTE pdrv, const BYTE* buff, DWORD sector, UINT count)
{
    ...
    UINT cur_sector = sector;
    /* TODO */
	if(ide_write_sectors(DISK_ID, count, sector, (unsigned int)buff) == 0){
		return RES_OK;
	}
	else{
		return -RES_ERROR;
