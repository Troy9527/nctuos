...
page_init(void)
{
    ...
    size_t i;
	for (i = 0; i < npages; i++) {
		if(i == 0 || (i>=PGNUM(IOPHYSMEM) && i<=PGNUM(EXTPHYSMEM)) || (i>=PGNUM(EXTPHYSMEM) && i<=PGNUM(PADDR(boot_alloc(0))))){
			pages[i].pp_ref = 1;
...
//
struct PageInfo *
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)  // return the physical page
{
    /* TODO */
...
setupkvm()
{
	...
	if(p = page_alloc(ALLOC_ZERO)){
		...
		boot_map_region(ret, KSTACKTOP-KSTKSIZE, ROUNDUP(KSTKSIZE,PGSIZE), PADDR(bootstack), (PTE_W | PTE_P));
