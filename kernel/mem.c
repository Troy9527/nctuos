...
mem_init(void)
{
	...
	//boot_map_region(kern_pgdir, KERNBASE, ROUNDUP(0x0fffffff,PGSIZE), 0, (PTE_W | PTE_P));
...
page_init(void)
{
	...
	size_t i;
	for (i = 0; i < npages; i++) {
		if(i == 0 || (i>=PGNUM(IOPHYSMEM) && i<=PGNUM(EXTPHYSMEM)) || (i>=PGNUM(EXTPHYSMEM) && i<=PGNUM(PADDR(boot_alloc(0)))) || i == PGNUM(MPENTRY_PADDR)){
			pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(*pde & PTE_P){
		...
		if(!create){
			...
			if(newpg == NULL){ // allocation fails
				return NULL;
			}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for(csize = 0; csize < size; csize += PGSIZE){
		...
		if(pte == NULL){
			panic("pgdir_wlak return NULL!\n");
		}
		else{
			*pte = PTE_ADDR(cpa) | PTE_P | perm;
		}
	}
...
/*	pde_t *ret = NULL;
	...
	if(p = page_alloc(ALLOC_ZERO)){
		...
		boot_map_region(ret, KERNBASE, ROUNDUP((0xffffffff-KERNBASE),PGSIZE), 0, (PTE_W | PTE_P));
	...
	int i = 0;
	for(i = 0; i<NCPU; i++){
		boot_map_region(ret, KSTACKTOP - i*(KSTKSIZE+KSTKGAP)-KSTKSIZE, ROUNDUP(KSTKSIZE,PGSIZE), PADDR(percpu_kstacks[i]), (PTE_W | PTE_P));
	}

	// MMIO region for lapic
	if(MMIOBASE + 4096 <= MMIOLIM){
		boot_map_region(ret, MMIOBASE, ROUNDUP(4096, PGSIZE), lapicaddr, (PTE_PCD | PTE_PWT | PTE_W));
	}
